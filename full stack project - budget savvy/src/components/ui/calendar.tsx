...
function Calendar({
  ...
  // Custom modifiers to highlight specific days
  const modifiers = {
    highlighted: highlightedDays,
  };

  // Custom day rendering function
  const handleRenderDay = (day: Date) => {
    if (renderDay) {
  ...
  return (
    <DayPicker
      ...
      modifiersClassNames={{
        highlighted: "bg-primary/10 rounded-md"
      }}
      components={{
        ...
        Day: (dayProps) => {
          ...
          if (!isCurrentMonth) {
            return <div className="rdp-day">{date.getDate()}</div>;
          }
          
          return <div className="rdp-day">{handleRenderDay(date)}</div>;
        },
      }}
      modifiers={modifiers}
      {...props}
